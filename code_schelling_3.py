...
... and see!
9 states model with non-uniform probability
"""
...
class Schelling:
    ...
    def run(self):
        ...
        for i in range(len(self.delinquant)) :
            ...
            if k == 1 : 
                if self.city[a][b]<0.8 : self.city[a][b]=self.city[a][b]+0.2
                elif self.city[a][b]==0.8 : self.city[a][b]=self.city[a][b]+0.1
                supp.append(i)
            else :
...
st.title("Spatial crime (9 states and non-uniform probability)")
